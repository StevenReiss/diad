...
class DiexecuteSetup implements DiexecuteConstants
{
...
private DiexecuteChangeData      changed_items;
private DiadStackFrame           active_frame;
private DiexecuteCall            active_call;
private List<DiexecuteAction>    action_set;
private DiexecuteCall            last_call;
...
DiexecuteSetup(DiexecuteManager ctx,DiexecuteChangeData cd,DiadStackFrame frm,DiexecuteCall vc)
{
   exec_manager = ctx;
   changed_items = cd;
   active_frame = frm;
...
DiexecuteSetup(DiexecuteSetup par,DiexecuteCall vc,Map<String,String> varmap)
{
   exec_manager = par.exec_manager;
...
List<DiexecuteAction> findResets()
{
      ...
      long time)
{
   ...
   DiexecuteCall usecall = null;
   
   for (DiexecuteCall vc : active_call.getInnerCalls()) {
      if (last_call != null) {
   ...
   DiexecuteSetup nsetup = new DiexecuteSetup(this,usecall,varmap);
   nsetup.findResets();
      ...
      List<?> args,List<String> argvals)
{
   ...
   DiexecuteChangeMap tcm = changed_items.getChanges(active_frame);
...
//       System.err.println("CHECK " + mthd.getNongenericName());
   ...
   if (useexpr != null) {
      DiexecuteAction act = DiexecuteAction.createInitAction(useexpr);
      action_set.add(act);
    }
}
...
private ASTNode getAstNodeForCall(DiexecuteCall vc)
{
   ...
   ASTNode n = srcmgr.getSourceNode(proj,
...
}       // end of class DiexecuteSetup




/* end of DiexecuteSetup.java */
